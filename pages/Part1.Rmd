...
For instance, if you found that in your dataset part of the variability is due to the different patients from which the biological replicates come from, but you want to assess other biologically driven differences due to a certain treatment, you could modify your design formula in order to **check differences due to the treatment, while controlling simultaneously for patient-to-patient variability**. In other words, to find genes that are differentially expressed in tumors compared to normal tissues, adjusting for baseline differences between the patients.

```{r, eval=FALSE, include=FALSE}

# We are not interested in difference between the patients but in the differences between tissues
# The design will estimate the patient effect
# The treatment, tumor effect is adjusted for differences between patients (or batches) by using an additive model formula.
#... so we need the columns to estimate the effect of each patient.

# From edgeR vignette: 
# Paired samples are a simple example of what is called ŞblockingŤ in experimental design. The
...
# The set of all normalization factors for a DGEList multiply to unity, ensuring that the geomet-
# ric mean of the effective library sizes is the same as the geometric mean of the original library
# sizes. A normalization factor below one indicates that a small number of high count genes
...
product(dds$samples$norm.factors)
```
...
# Compute z-score by row (every gene gets mean=0 and sd=1) 
zscore <- t(apply(log2dds, 1, function(x) (x - mean(x))/sd(x) ) )

zscore <- t(scale(t(log2dds)))

zscore %>% head(2)

# Mean should 0, sd should be 1
apply(zscore, 1, mean ) %>% head(10)

# Max and min values are not the same
apply(zscore, 1, min ) %>% head(10)
